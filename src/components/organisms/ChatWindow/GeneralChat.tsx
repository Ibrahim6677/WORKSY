...
function isReplyMessage(msg: Message): msg is Message & {
  replies: number;
  lastReply: string;
  replyAvatars: string[];
} {
  return (
    typeof msg.replies === "number" &&
    !!msg.lastReply &&
    Array.isArray(msg.replyAvatars)
  );
}
function isThreadMessage(
  msg: Message
): msg is Message & { thread: boolean; replyAvatars: string[] } {
  return Boolean(msg.thread) && Array.isArray(msg.replyAvatars);
}
...
const mockMessages: MessageSection[] = [
  {
    ...
    messages: [
      ...
      },
      {
        ...
        time: "4:49 PM",
        text: "Anyone in Fort miami area?",
        replies: 4,
        lastReply: "today at 8:43 AM",
        replyAvatars: [
  ...
  },
  {
    ...
    messages: [
      ...
      },
      {
        ...
        time: "2:49 PM",
        text: "Anyone in Belfast or around?",
        thread: true,
        replyAvatars: [
...
const GeneralChat = () => {
  ...
  const [messages, setMessages] = useState<MessageSection[]>(mockMessages);
              ...
              {section.messages.map((msg) => {
                return (
                  <div
                    key={msg.id}
                    className={
                      isThreadMessage(msg) ? "bg-[#F6F6F6] rounded-xl p-4" : ""
                    }
                  >
                      ...
                      <div className="flex-1">
                        ...
                        {isReplyMessage(msg) && (
                          <div className="flex items-center gap-2 mt-2">
                            ...
                            <span className="text-xs text-[#6629DE] font-medium cursor-pointer">
                              {msg.replies} replies
                            </span>
                            <span className="text-xs text-[#AFB8CF]">
                              Last reply {msg.lastReply}
                            </span>
                          </div>
                        )}
                        {isThreadMessage(msg) && (
                          <div className="flex items-center gap-2 mt-3 bg-white rounded-lg border border-[#EFEFFD] px-3 py-2">
                            {msg.replyAvatars?.map((a, i) => (
                              <img
                                key={i}
                                src={a}
