...
const thread = {
  ...
  main: {
    ...
    time: "20 hours ago",
    text:
      "Btw, getting this messaging on this Slack instance not sure if anybody the team needs to see this, or perhaps you did a trial and are just waiting for it to run down?",
  },
  replies: [
    {
      name: "Scott Franklin",
      ...
      time: "4:49 PM",
      text:
        "Am I correct that if I go on vacation or otherwise need to block my availability for mentoring, I need to do it individually for each session that I've set up - even if I don't have multiple session types created?",
    },
  ],
};
...
const ThreadSidebar = ({ onClose }: ThreadSidebarProps) => {
    ...
    const [messages, setMessages] = useState<MessageSection[]>([
      {
        ...
        messages: [
          {
            id: 1,
            name: thread.main.name,
            avatar: thread.main.avatar,
            time: thread.main.time,
            text: thread.main.text,
          },
  ...
  return (
    <div className="flex flex-col h-full w-full pb-6 bg-white">
      {/* Header */}
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
        <div className="flex items-baseline gap-2">
          <span className="font-semibold text-lg text-gray-900">Thread</span>
          <span className="text-sm text-gray-400">{thread.channel}</span>
        </div>
      ...
      {/* Main message and replies (dynamic) */}
      <div className="px-6 pt-4 pb-2 text-xs text-gray-500">{messages[0].messages.length - 1} replies</div>
      <div className="flex-1 overflow-y-auto px-6 pb-4">
        {messages[0].messages.map((msg, idx) => (
          <div key={msg.id} className={idx === 0 ? "flex items-start gap-3 mb-6" : "flex items-start gap-3 mb-6 pl-6"}>
            <img
              src={msg.avatar}
              alt={msg.name}
              className={idx === 0 ? "w-10 h-10 rounded-full object-cover" : "w-9 h-9 rounded-full object-cover"}
            />
            <div>
